...
- `rate_limit_hits`, `pending_uploads`: shared-DB backing for the brute-force/spam throttle (`services/throttle.py`) and the presigned-upload ownership store — they replace the old in-memory state so limits and upload keys hold across workers/replicas (both created by `create_all`; no shim needed).
...
- `components/ThemeToggle.tsx`: the light/dark switch, mounted in `components/Nav.tsx` and the landing header; flips `data-theme` and persists the choice.
...
Next.js 15 App Router requires `"use client"` for hooks such as `useParams`, `useSearchParams`, and auth state. The design system is **"Noir"** — true black/white neutrals with one magnetic-pink accent used sparingly (pink `--color-accent` acts — CTAs / live signals; ice blue `--color-accent-2` informs — insights / secondary), display face Schibsted Grotesk. TWO first-class themes ship: dark (default; near-black `#0A0A0B`, pink `#FF4D8D`) and light (paper white, deep raspberry `#D6246E`), switched by `data-theme` on `<html>` — `components/ThemeToggle.tsx` flips it, a pre-paint script in `app/layout.tsx` restores the `surge-theme` localStorage choice with no flash, and every visual change must be verified in BOTH themes. All colors are CSS tokens in `app/globals.css` (`:root` = dark, `[data-theme="light"]` overrides) so a palette change cascades app-wide. The core token ramp carries no gradients, but a SEPARATE `@layer utilities` platform-colorway layer (TikTok glitch `#25F4EE`/`#FE2C55`, Instagram gradient) is intentionally on-brand on hero surfaces + headings — do not fold those brand colors into the neutral ramp, and their motion is HOVER-ONLY by product decision: the TikTok chromatic split/glitch bursts and the Instagram gradient rotation run only while the cursor is over the wordmark. Verdict display labels/colors come from `lib/verdicts.ts`. PWA assets are under `frontend/public`.
