...
Across ALL users, computes the correlation between each craft dimension score and
the observed like-rate at a FIXED maturity window, WITH n and a 95% confidence
interval, plus naive baselines (caption length, posting hour) for comparison, and
the inter-dimension correlation matrix (P2-A collinearity check).
...
def _print_report(rep: dict) -> None:
    ...
    for b in rep.get("baselines", []):
        ...
        print(f"  {b['baseline']:<20} {b['n']:>4} {rr:>8}   {_fmt_ci(b['ci95'])}{flag}")
